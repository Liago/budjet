...
import { addMonths, startOfMonth, endOfMonth, format, subMonths, subDays, startOfDay, endOfDay, isValid } from 'date-fns';
...
export function useDashboardDateRange(): UseDashboardDateRangeResult {
  ...
  const [selectedTimeRange, setSelectedTimeRange] = useState<TimeRange>('current-month');
  
  // State for custom date range
  const [customStartDate, setCustomStartDate] = useState(format(subMonths(today, 1), 'yyyy-MM-dd'));
  ...
  const applyCustomDateRange = () => {
    try {
      ...
      console.error('Errore nell\'applicare il range di date personalizzato:', error);
    }
  };
