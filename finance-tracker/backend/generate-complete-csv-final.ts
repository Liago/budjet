...
console.log('🚀 Generazione CSV COMPLETO con TUTTE le relazioni Tag-Transazioni');
...
relations.forEach((relation: any) => {
  ...
  if (tagExists && transactionExists) {
    ...
    validRelations++;
    console.log(`✅ Tag: ${tagId.slice(0,8)}... → Transaction: ${transactionId.slice(0,8)}...`);
  } else {
    invalidRelations++;
    if (invalidRelations <= 5) { // Mostro solo i primi errori per non intasare
      console.log(`❌ Relazione ignorata: Tag ${tagExists ? '✓' : '✗'} Transaction ${transactionExists ? '✓' : '✗'}`);
    }
  }
});
