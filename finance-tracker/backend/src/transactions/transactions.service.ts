...
export class TransactionsService {
  ...
  async findAll(userId: string, queryDto: QueryTransactionDto) {
    const { startDate, endDate, type, categoryId, tag, page = 1, limit = 10 } = queryDto;
  ...
  async removeAllForUser(userId: string) {
    try {
      ...
      throw new BadRequestException(`Failed to delete transactions: ${error.message}`);
    }
  }

  async importFromCsv(userId: string, file: Express.Multer.File, defaultCategoryId?: string) {
    if (!file) {
    ...
    if (!defaultCategoryId) {
      ...
      defaultCategory = userCategories.find(cat => cat.name.toLowerCase() === 'uncategorized');
    ...
    return new Promise((resolve, reject) => {
      ...
      parser.on('readable', async () => {
        ...
        while ((record = parser.read()) !== null) {
          try {
            ...
            let date;
            try {
              date = record.Date ? parseDate(record.Date, 'MMM d, yyyy', new Date()) : new Date();
            } catch (error) {
              date = new Date();
            }
            ...
            const tags = note.match(/#(\w+)/g)?.map(tag => tag.substring(1)) || [];
            ...
            results.push({
              ...
              date,
              description: note.replace(/#\w+/g, '').trim() || `Imported ${type.toLowerCase()}`,
      ...
      parser.on('end', async () => {
        try {
          ...
          reject(new BadRequestException(`Failed to import transactions: ${error.message}`));
        }
      });
      ...
      try {
        ...
        reject(new BadRequestException(`Error processing file: ${error.message}`));
      }
    });
  }
} 